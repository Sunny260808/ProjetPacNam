...
## Se familiariser avec l'environement

Vous êtes sur i3 : un gestionaire de fenêtre qui marche par tuile. Fini les fenêtres flotantes de Windows !
...
Pour commencer, exécutez la commande `./setup.sh` qui permettera d'installer l'environement

Une fois que la commande est terminé, pour lancer Visual Studio Code tapez `code` dans le terminal

Pour lancer votre programme python, allez dans le dossier `PacNam/` et exécutez `python main.py`

Vous avez les fichiers `pacnam.py` et `ghost.py` a compléter.

## Le fichier `pacnam.py`

Vous avez les atttributs suivant à votre disposition : 
...
- `self.dt`           (Le delta de temps, pour adapter le mouvement à la fréquence d'image (vous n'avez pas a le modifier))
...
- `self.power_up`     (Vous dis si PacNam a son power_up (True) ou pas (False))

- `self.timer`        (Durée de recupération du power-up)

- `self.make.maze`    (Un tableau à deux dimension (une liste de liste) qui représente les murs (1) et les espaces ou PacNam peux aller (0))

**Note** : Pour accéder à une case en particulier, vous pouvez faire ```python self.maze.maze[ligne][colonnes]```

- `self.maze.point`  (Même structure que précédement, mais qui représente les points (1) les power-ups (2) et les espaces vides (ni points ni power-up) (0))

Dans ce fichier, vous devrez faire les méthodes : 

- `move_*()`         (Permet au PacMan de bouger à gauche/droite/haut/bas dans le labirythe, en testant s'il vas dans un mur. Vous devez modifier les variables `self.pos.x` et `self.pos.y`. *Vous devrez utiliser self.dt dans cette fonction en la multipliant par la vitesse du PacNam*)
...
- `kill`              (Le Fantôme a été touché, quand PacNam avait sont power_up. Il doit retourner a sont point de départ et son chemin a été réinitialisé)

- `action()`          (Soit vous faite déplacer le fantôme au hasard sur la grille, soit vous lui faite suivre l'algorithme A*)
